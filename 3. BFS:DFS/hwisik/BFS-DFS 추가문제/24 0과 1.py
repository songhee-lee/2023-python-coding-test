...
def find_nineapple_number(n):
    ...
    while queue:
        ...
        with_zero = cur_num * 10
        with_one = cur_num * 10 + 1
        
        if not visited[with_zero % n]:
            visited[with_zero % n] = 1
            queue.append((with_zero % n, cur_str + '0'))
        
        if not visited[with_one % n]:
            visited[with_one % n] = 1
            queue.append((with_one % n, cur_str + '1'))
    
t = int(input())

for _ in range(t):
    n = int(input())
    print(find_nineapple_number(n))
...
#         # 모듈러 연산을 활용해서 n으로 나눈 나머지를 큐에 추가한다.
#         # cur_str은 추가적인 연산없이 그대로 사용한다.
